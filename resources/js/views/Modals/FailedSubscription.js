...
class FailedSubscription extends Component {
    constructor(props) {
        super(props);
        this.state = {
            reason: ''
    ...
    componentDidMount() {
        Emitter.on('openSubscriptionFailedModal', (data = null) => {
            if (data) {
                this.setState({
                    reason: data
                })
            }
            $('#subscriptionFailed').modal('show');
        })
    }

    closeModal() {
        $('#subscriptionFailed').modal('hide');
    }

    render() {
        return (
            <div>
                <div className="modal fade" id="subscriptionFailed" tabIndex="-1" role="dialog"
                            ...
                            <div className="modal-body">
                                ...
                                <div className="container mt-3">
                                    ...
                                    <div className={`row`}>
                                        ...
                                        <div className={'col-12 d-flex justify-content-center'}>
                                            <div className={'mt-1'}>
                                                <h2 className={'font-weight-bold text-center'}>Subscription failed</h2>
                                                <h6 className={`text-center`}>{this.state.reason ? this.state.reason :
                                                    "You don't have enough funds to make this payment"}</h6>
                                            </div>
                                        </div>
